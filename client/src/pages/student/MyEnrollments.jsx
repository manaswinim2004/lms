    ...
    try {
      ...
      const tempProgressArray = await Promise.all(
        enrolledCourses.map(async (course) => {
          ...
          // Use totalLectures from backend if available
          let totalLectures = data.totalLectures || calculateNoOfLectures(course);
          const lectureCompleted = data.progressData ? data.progressData.lectureCompleted.length : 0;
          ...
          <tbody className='text-gray-700'>
            {enrolledCourses.map((course, index) => (
              <tr key={index} className='border-b border-gray-500/20'>
                ...
                <td className='px-4 py-3 max-sm:hidden'>
                  {progressArray[index] ? `${progressArray[index].lectureCompleted} / ${progressArray[index].totalLectures} Lectures` : '0 / 0 Lectures'}
                </td>
                <td className='px-4 py-3 max-sm:text-right'>
                  <button 
